from src.strfem import Controller


def sample_frame(controller: Controller):
    # Nodes
    ...
    # Supports
    support1 = controller.add_support_fixed("Fixed")

    controller.apply_support(node1, support1)
    controller.apply_support(node4, support1)

    # Sections
    section1 = controller.add_section_rect("300x500", 0.3, 0.5)

    controller.apply_section(line1, section1)
    controller.apply_section(line2, section1)
    controller.apply_section(line3, section1)

    # Materials
    material1 = controller.add_material("Concrete", 20e9, 10e9, 0.2)

    controller.apply_material(line1, material1)
    controller.apply_material(line2, material1)
    controller.apply_material(line3, material1)

    # Releases
    release1 = controller.add_release_pinned_rigid("Pin-Rigid")

    controller.apply_release(line2, release1)
    ...
    line_load_dist1 = controller.add_line_load_dist(
        load_case1.id, x_span=0.2, Fz_start=-5000, Fz_end=-5000
    )

    controller.apply_line_load(line_load_dist1, line2, 0.4)


def sample_frame_test(controller: Controller):
    # Nodes
    node1 = controller.add_node([0, 0, 0])
    node2 = controller.add_node([0, 0, 5])
    node3 = controller.add_node([10, 0, 5])
    ...
    # Materials
    material1 = controller.add_material("Steel", 200e9, 75e9, 0.3)
    material2 = controller.add_material("Aluminium", 70e9, 10e9, 0.2)
    material3 = controller.add_material("Concrete", 20e9, 10e9, 0.2)
